...
export function SearchStats({ className, showDetailed = true }: SearchStatsProps) {
  ...
  const calculateStats = () => {
    ...
    const totalResults = filteredHistory.reduce((sum, search) => sum + search.count, 0);
    ...
    const queryWords = filteredHistory
      .flatMap(search => search.query.toLowerCase().split(' '))
      .filter(word => word.length > 2);
    
    const wordCounts = queryWords.reduce((acc, word) => {
    ...
    const topQueries = Object.entries(wordCounts)
      ...
      .slice(0, 5)
      .map(([word, count]) => ({ label: word, value: count, percentage: Math.round((count / queryWords.length) * 100), color: 'bg-blue-500' }));
